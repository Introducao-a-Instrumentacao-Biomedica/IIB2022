...
char default_database[] = "newWordPressDb";           //"test_arduino";
char default_table[]    = "esp32";         //"test_arduino";

// Sample query
char INSERT_DATA[] = "INSERT INTO %s.%s (Temperatura, BPM, Sp02) VALUES (%s,%s,%s)";
                 
char query[128];
char temperature[10];
char BPM[10];
char Sp02[10];
float val_temperatura,val_BPM,val_Sp02;
...
void setup()
{
  ...
  Serial.println("\nStarting Complex_Insert_WiFi on " + String(BOARD_NAME));
...
void runInsert(void)
{
  ...
  val_BPM = 140 + (rand() % 9);
  val_Sp02 = 95 + (rand() % 4);
  val_temperatura = 38 + (rand() % 1);

  if (conn.connected())
  {
    // Save
    dtostrf(val_temperatura,4, 2, temperature);
    dtostrf(val_Sp02, 4, 2, Sp02);
    dtostrf(val_BPM,5, 1, BPM);
    sprintf(query, INSERT_DATA, default_database, default_table, temperature , BPM, Sp02);
